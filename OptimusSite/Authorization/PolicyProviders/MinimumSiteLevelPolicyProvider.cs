...
namespace OptimusSite.Authorization.PolicyProviders
{
    internal class MinimumSiteLevelPolicyProvider : IAuthorizationPolicyProvider
    {
        private const string POLICY_PREFIX = "AuthorizeMinimumSiteLevel";
        public DefaultAuthorizationPolicyProvider FallbackPolicyProvider { get; }

        public MinimumSiteLevelPolicyProvider(IOptions<AuthorizationOptions> options)
        {
        ...
        public Task<AuthorizationPolicy> GetPolicyAsync(string policyName)
        {
            if (policyName.StartsWith(POLICY_PREFIX, StringComparison.OrdinalIgnoreCase) &&
                int.TryParse(policyName.Substring(POLICY_PREFIX.Length), out var siteLevel))
            {
                var policy = new AuthorizationPolicyBuilder();
                policy.AddRequirements(new MinimumSiteLevelRequirement(siteLevel));
